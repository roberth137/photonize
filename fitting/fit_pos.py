...
def avg_of_roi_w_bg(localization, bg_pixel, phot_locs, box_side_length, return_sd=False):
    """
    Parameters
    ----------
    phot_locs : photons of one localization as pd dataframe
    ...
    if return_sd:
        ...
        sd_x = calculate_sd_w_bg(x_photons, pos_x, total_photons, bg_photons, box_side_length)
        sd_y = calculate_sd_w_bg(y_photons, pos_y, total_photons, bg_photons, box_side_length)
...
def calculate_sd_w_bg(positions, pos_fit, total_phot, bg_total, diameter):
    ...
    """
    #bg_var = ((diameter/4) ** 2) / 4  # s.d. in 1d of a random distribution on disk
    var = np.sum((positions - pos_fit) ** 2)/total_phot # - (bg_var * bg_total)) / number_phot
    return 10 if var <= 0 else np.sqrt(var)
...
def event_position_w_bg(event, phot_event, diameter, return_sd=True):
    ...
    if return_sd:
        ...
        sd_y = calculate_sd_w_bg(y_photons, pos_y, total_photons, bg, diameter)
