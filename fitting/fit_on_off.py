...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    # Simulate a burst of photons: background + event
    background = np.random.uniform(0, 500, size=500)
    event = np.random.normal(loc=1000, scale=100, size=1500)
    all_times = np.concatenate([background, event])
    all_times = all_times[(all_times >= 0) & (all_times <= 2000)]

    photons = PhotonMock(ms_array=all_times)

    start, end, duration = get_on_off_dur(photons, bin_size=10, smoothing_size=5)
    ...
    bins = np.arange(min(photons.ms), max(photons.ms) + 50, 50)
    counts, _ = np.histogram(photons.ms, bins=bins)
    smoothed = lee_filter_1d(counts, 5)
    centers = (bins[:-1] + bins[1:]) / 2

    plt.figure(figsize=(8, 5))
    plt.bar(centers, counts, label='Original Histogram', alpha=0.5)
    plt.bar(centers, smoothed, label='Smoothed (Lee Filter)', linewidth=10)
