...
def locs_lt_to_picasso(localizations_file, photons_file, drift_file, offset, fitting='avg', bsl=5, int_time=200):
    '''
    ...
    for g in range((localizations['group'].iloc[-1])+1):
        ...
        phot_cr_und = undrift(phot_crop, drift, int_time)
        ...
        for i in range(len(locs_group)):
            if i == 0:
                phot_loc = get_photons(locs_group.iloc[i], phot_cr_und_cr, offset, 5 ,200)
...
def get_photons(localization, photons_file, offset=1, box_size=5, int_time=200):
    '''
    ...
    start_ms = ((localization.frame/offset)*int_time)
    ms_min, ms_max = start_ms, start_ms+int_time
...
def phot_of_locs(localizations_file, photons_file, drift_file, bsl=5, int_time=200):
    '''
    ...
    for g in range((localizations['group'].iloc[-1])+1):
        ...
        for i in range(len(locs_group)):
            if i == 0:
                loc_photons = get_photons(locs_group.iloc[i], phot_cr_und_cr, box_size=bsl, int_time=int_time)
                ...
                loc_photons2 = get_photons(locs_group.iloc[i], phot_cr_und_cr, box_size=bsl, int_time=int_time)
...
def undrift(photons_index, drift_file, int_time=200):
    '''
    IN: 
    ...
    frames = np.floor(ms_index/int_time).astype(int)
...
def photons_to_picasso(photons_dataframe, int_time=200):
    ...
    for i in range(len(photons_dataframe)):
        frame_arr[i] = int(math.floor(phot_frame.ms[i]/int_time))
