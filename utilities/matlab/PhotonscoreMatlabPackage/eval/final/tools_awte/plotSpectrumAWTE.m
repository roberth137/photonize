...
%    style = style.style;
    ...
    if size(spectrum,2)==1
        ...
        textColor = jet(size(spectrum,2))*0.7;
        specColor = turbo(size(spectrum,3))*0.9;
    end
...
%     hold off;
    ...
    for nn = 1:size(spectrum,2)
        for it = it_ids
            ...
            % spectrum
          p{nn} = semilogx(tau_range, x, 'Color', textColor(nn,:), 'LineWidth',2);
            ...
            peakColor = lines(length(tauData));
...
%                             'HorizontalAlignment', 'center', 'Color', textColor(nn,:))
                 ...
                 text(tauData(tt).byMax, tauData(tt).peakHeight, txtStr, ...
                            ...
                            'FontSize', 12, 'Color', textColor(nn,:))
    ...
    if (axisLim(2) > 0)
        xlim1 = floor(log10(tau_range(1)));
        axis([10^xlim1 tau_range(end) 0 axisLim(2)*2])
    end
    
    if (nargin>2), legend([p{:}],legendStr); end
