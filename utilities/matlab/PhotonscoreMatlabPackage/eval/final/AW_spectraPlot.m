function   [tau,Normalpha, COUNTS] = AW_spectraPlot(spectrum, tau_range_exp, ch, iterations, B, ...
                                        do_print, ownFigure,  holdFigure)
                                    
%% [tau,Normalpha, COUNTS] = AW_spectraPlot(spectrum, tau_range_exp, ch, iterations, B, ...
%%                                         do_print, ownFigure,  holdFigure)
%% addpath of Style 
...
for i=1:size(spectrum,1)
    if s(i)==-1000 && s(i-1)==0 || s(i)==-1000 && s(i-1)==1000 
        vektor1(j,kk)=i;
        j=j+1;
    end
...
for i=1:size(vektor2,1)
   if isnan(vektor1(i,kk))==0 && vektor1(i,kk)>0
       if vektor1(i, kk)-vektor2(i, kk)<0 || size(vektor1)>size(vektor2)
           warning ('something wrong - with borders')
           continue
       end
...
title(B{kk})
        ...
        if do_print
            exportFigure( ['Est_Lifetimes_',strrep(B{kk},' ', '_'),...
                           '_',num2str(iterations(kk)),'it_', '.png'], 300);
            saveas(gcf, ['Est_Lifetimes_' ,strrep(B{kk},' ', '_'),...
                        '_',num2str(iterations(kk)),'it_','.fig'], ...
                          'fig');
        end
